...
import Like from "@/assets/icon/like_icon.svg";
import DisLike from "@/assets/icon/dislike_icon.svg";
import Comment from "@/assets/icon/comment-icon.svg";

const url = import.meta.env.VITE_BASE_URL;
const commentSectionID = ref(null);
...
const openCommentModal = () => {
    show.value = !show.value; // Close modal
};

const openCommentSection = (e) => {
    commentSectionID.value = e.id
    console.log(e);
    commentId.value = e.id
    commentUserId.value = e.userId
...
const onSubmit = async (e) => {
    e.preventDefault();
    try {
        await axios.post(`${url}/api/Comment/Add/addComment/api`, {
            senderId: commentUserId.value,
            itemId: commentId.value,
...
<template>
    <div class="grid grid-cols-3 gap-4">
        <div v-for="item in posts" :key="item.id" class=" border border-[#000]">
            <img src="https://images.unsplash.com/photo-1420593248178-d88870618ca0?q=80&w=1000&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8bmF0dXJhbHxlbnwwfHwwfHx8MA%3D%3D" alt="Blog image" class="" />
            <div class="px-[15px] py-[20px]">
                <div>
                    <h6>{{ item.name }}</h6>
                    <p class="blog-date">{{ dayjs(item.createAt).format('YYYY-MM-DD') }}</p>
                    <h2 class="py-[20px] text-[25px] max-w-[310px] font-mono">{{ item.title }}</h2>
                    <p class=" leading-[25px] font-serif">{{ item.description }}</p>
                </div>
                <div class="w-[100%] h-[1px] bg-black my-[20px]"></div>
                <div class="flex flex-col gap-[20px]">
                    <div class="flex items-center justify-between">
                        <div @click="like" class="flex items-center cursor-pointer gap-[5px]">
                            <svg :fill="fillColor" height="30px" width="30px" version="1.1" id="Capa_1"
		...
		C482.323,308.985,475.927,319.848,465.561,325.727z" />
                            ...
                            <h6 class="text-[18px] font-semibold">Like {{ likesCount }}</h6>
                        </div>
                        <div @click="dislike" class="flex items-center gap-[5px] ">
                            <img :src="DisLike" alt="" class="w-[25px] h-[25px] mt-[7px]">
                            <h6 class="text-[18px] font-semibold">DisLike {{ dislikeCount }}</h6>
                        </div>
                        <div @click="() => openCommentSection(item)" class="cursor-pointer flex items-center gap-[5px]">
                            <img :src="Comment" alt="" class="w-[25px] h-[25px]">
                            <h6 class="text-[18px] font-semibold">comments</h6>
                        </div>
                    </div>
                    <div v-show="commentSectionID === item.id && openComment" class="border p-2">
                        <p v-for="(content, index) in item?.comments">
                            {{ index + 1 }} - {{ content.content }}
                        </p>
                        <form @submit="onSubmit">
                            <textarea v-model="commentText" placeholder="Enter a comment"
                                class="border-red-400 border w-full mt-8 py-4 p-2" type="text">
                            </textarea>
                            <button class="border-black border px-3 py-1 mt-4" type="submit">Send</button>
                        </form>
                    </div>
                </div>
            </div>
